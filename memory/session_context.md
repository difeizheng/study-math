...
1. **性能优化**
   - 为 exam_scores 表添加 4 个索引（student_id, semester, exam_name, 复合索引）
   - DataManager 添加 @st.cache_data 缓存（5 分钟 TTL）
   - 缓存 get_students(), get_exam_list(), get_scores() 方法

2. **测试覆盖率提升**
   - 新增 test_data_manager.py: DataManager 单元测试（10 个测试）
   - 新增 test_score_config.py: 分数段配置测试（6 个测试）
   - 新增 test_knowledge_graph.py: 知识图谱测试（7 个测试）
   - 修复 knowledge_graph.py 缩进错误
   - 总计 65 个测试全部通过
